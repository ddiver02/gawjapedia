...
function transformRawDataToSnack(raw: SnackRawData): Snack {
    const nutrition: NutritionInfo = {
        ...
        fat: toNumber(raw['지방(g)']),
        transFat: toNumber(raw['트랜스지방(g)']),
        saturatedFat: toNumber(raw['포화지방(g)']),
        cholesterol: toNumber(raw['콜레스테롤(mg)']),
        protein: toNumber(raw['단백질(g)']),
        lactose: toNumber(raw['유당(g)']),
        calcium: toNumber(raw['칼슘(mg)']),
